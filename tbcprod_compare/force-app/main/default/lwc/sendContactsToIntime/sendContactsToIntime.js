...
export default class SendContactsToIntime extends LightningElement {
    // PRIVATE ATTRIBUTES 
    _recordsList;
    _recordsToDisplay;
    _dataList;
    ...
    **/
    handleSave (event) {
        ...
        saveRecords({
            'dataTableParams' : {
                'className' : 'DatatableController.PersonIntimeSendController',
                'contactIds' : JSON.stringify(this._selectedRecordIds)
            }
        }).then(result => {
            if (result && result.hasOwnProperty('apexJobId')) {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        "message" : "Selected contact(s) has/have been sent it to time successfully",
                        "variant" : 'success'
                    }),
                );
                this.getContacts();
