...
export default class RevertExpensesInIntime extends LightningElement {
    ...
    **/
    handleSave (event) {
        ...
        saveRecords({
            'dataTableParams' : {
                'className' : 'DatatableController.ExpenseRevertController',
                'expenseIds' : JSON.stringify(this._selectedRecordIds)
            }
        }).then(result => {
            // show success message
            this.dispatchEvent(
                new ShowToastEvent({
                    "message" : "Selected expenses has/have been sent it to intime successfully",
                    "variant" : 'success'
                }),
            );
            this.getClients();
