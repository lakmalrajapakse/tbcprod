...
export default class ShiftManagementAdjustmentSelector extends NavigationMixin(LightningElement) {
    ...
    handleTimesheetSearch(event) {
        this._filteredData = this.data;
        // WORKER
        if (event && event.detail && event.detail.worker) {
        ...
        // CHARGE RATE
        if (event && event.detail && event.detail.chargeRate) {
            this._filteredData = this._filteredData.filter(timesheet => timesheet.ChargeRate == event.detail.chargeRate);
        } 

        // PAY RATE 
        if (event && event.detail && event.detail.payRate) {
            this._filteredData = this._filteredData.filter(timesheet => timesheet.PayRate == event.detail.payRate);
        } 
    }
