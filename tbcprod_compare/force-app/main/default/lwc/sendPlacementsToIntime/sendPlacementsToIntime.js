...
export default class SendPlacementsToIntime extends LightningElement {
    // PRIVATE ATTRIBUTES 
    _recordsList;
    _recordsToDisplay;
    _dataList;
    ...
    **/
    ...
    /**
    * @description Method to fetch the placements
    **/
    getPlacements() {
        this._currentPage = 1;
        this._selectedEntry = 25;
    ...
    **/
    handleSave (event) {
        ...
        saveRecords({
            'dataTableParams' : {
                'className' : 'DatatableController.PlacementIntimeSendController',
                'placementIds' : JSON.stringify(this._selectedRecordIds)
            }
        }).then(result => {
            if (result && result.hasOwnProperty('apexJobId')) {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        "message" : "Selected placements(s) has/have been sent it to time successfully",
                        "variant" : 'success'
                    }),
                );
                this.getPlacements();
